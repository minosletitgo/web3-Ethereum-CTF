...
# 在实例部署过程中，使用了所有者私钥生成的两个签名：
# 切换锁签名（SWITCH_LOCK_SIG）：
...
# 可以看到，两个签名的r值相同，这意味着签名时使用了相同的随机数k。
# 这是ECDSA算法中的一个严重漏洞，攻击者可以利用该漏洞恢复出私钥（参考PS3的ECDSA漏洞事件）。

# 根据ECDSA签名生成算法：
# https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm#Signature_generation_algorithm.

# s_1 = k^(-1) * (z_1 + r_1 * 所有者私钥) mod n
# s_2 = k^(-1) * (z_2 + r_2 * 所有者私钥) mod n

# 由于r_1 = r_2 = r，统一赋值
r = r_1

# 由此推导随机数k：
# s_1 - s_2 = k^(-1) * (z_1 + r * 所有者私钥) - k^(-1) * (z_2 + r * 所有者私钥) mod n
# s_1 - s_2 = k^(-1) * (z_1 + r * 所有者私钥 - z_2 - r * 所有者私钥) mod n
# s_1 - s_2 = k^(-1) * (z_1 - z_2) mod n
# k = (s_1 - s_2)的模逆 * (z_1 - z_2) mod n
k = mod_inverse(s_1 - s_2, secp256k1.n) * (z_1 - z_2) % secp256k1.n

# 一旦获取到k，就可以恢复出所有者私钥：
# 所有者私钥 = r的模逆 * (s_1 * k - z_1) mod n
owner_sk = mod_inverse(r, secp256k1.n) * (s_1 * k - z_1) % secp256k1.n

# 然后我们可以生成有效的签名来解锁合约并将管理员权限修改为自己
...
# 检查s值是否超过曲线阶的一半，若超过则使用其补数（符合BIP-0062规范）
