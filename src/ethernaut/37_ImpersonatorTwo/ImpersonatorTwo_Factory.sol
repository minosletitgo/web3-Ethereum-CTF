...
contract ImpersonatorTwo_Factory is Level {
	...
	function createInstance(address _player) public payable override returns (address) {
		...
		ImpersonatorTwo impersonatorTwo = new ImpersonatorTwo{value: msg.value}();
		// Set Bob as owner
		impersonatorTwo.transferOwnership(OWNER);
		// Lock the funds
		impersonatorTwo.switchLock(SWITCH_LOCK_SIG);
		// Set Alice as admin by using the signature crafted by Bob
		impersonatorTwo.setAdmin(SET_ADMIN_SIG, ADMIN);
		
		instance = address(impersonatorTwo);
	...
	function validateInstance(address _player) public view override returns (bool) {
		_player; // 消除警告
		ImpersonatorTwo inst = ImpersonatorTwo(payable(instance));
