...
contract EllipticToken_Factory is Level {
	...
	function createInstance(address _player) public payable override returns (address) {
		...
		EllipticToken ellipticToken = new EllipticToken();
		ellipticToken.transferOwnership(BOB);
		...
		ellipticToken.redeemVoucher(INITIAL_AMOUNT, ALICE, salt, bobSignature, aliceSignature);
		
		instance = address(ellipticToken);
	...
	function validateInstance(address _player) public view override returns (bool) {
		_player; // 消除警告
		EllipticToken inst = EllipticToken(payable(instance));
		return (inst.balanceOf(ALICE) == 0);
	}
}
